...
from db.models import UserAirports, UserStates, UsersTranzit
...
class UserState:
    ...
    def state(self) -> UserStates:
        user = Users.get(user_id=self.search_request_data.user_id)
        return user.states

    @state.setter
    def state(self, new_state: UserStates) -> None:
        query = Users.update({Users.states: new_state.value}).where(Users.user_id == self.search_request_data.user_id)
        query.execute()
...
def run_bot() -> None:
    ...
    # Here is stored instance of class that can be accessed by user id

    users_state: dict[int, UserState] = {}

    refresh_states(users_state)
    ...
    def send_welcome(message: types.Message):
        ...
        Users.create_from_message(message)
    ...
    def compute_route_handler(callback_query):
        """
        Here is created instance class of UserState where will store all data about user.
        ...
        """
        Users.create_from_message(callback_query.message)
        ...
        bot.reply_to(
            ...
            "Напиши название города отправления. \n\n<i>Например - Москва или " "Санкт-Петербург</i>.",
            parse_mode="HTML",
        )

    @bot.message_handler(
        func=lambda message: message.chat.id in users_state
        and users_state[message.chat.id].state == UserStates.WAIT_FOR_AIRPORT
    )
    ...
    @bot.message_handler(
        func=lambda message: message.chat.id in users_state
        and users_state[message.chat.id].state == UserStates.WAIT_FOR_TRANSIT_PERIOD
    )
    ...
    @bot.message_handler(
        func=lambda message: message.chat.id in users_state
        and users_state[message.chat.id].state == UserStates.WAIT_FOR_HATE_AIRL
    )
    ...
    def start_search_handler(callback_query):
        ...
        try:
            ...
            sr = Search()
            start_date, end_date, airports, start_period, end_period, home, finish, tranzit, hate_airl = users_state[
                callback_query.message.chat.id
            ].search_request_data.start()
            bot.send_message(
                callback_query.message.chat.id,
    ...
    @bot.message_handler(
        func=lambda message: message.chat.id in users_state
        and users_state[message.chat.id].state == UserStates.WAIT_FOR_HOME
    )
    def home_handler(message: types.Message):
        """
        ...
        if answer == True:
            ...
            bot.send_message(message.chat.id, text=f"Выбери дату или период вылета.", reply_markup=calendar)
        else:
            bot.send_message(
                message.chat.id,
    ...
    def call_handler(c):
        ...
        if not result and key:
            try:
                bot.edit_message_text(f"Выбери:", c.message.chat.id, c.message.message_id, reply_markup=key)
            except:
                pass
            ...
            answer_bool = users_state[c.message.chat.id].search_request_data.set_start_date(
                first_value=result, second_value=None
            )
            if answer_bool:
                markup = types.InlineKeyboardMarkup()
            ...
            answer = users_state[c.message.chat.id].search_request_data.append_date_or_period_to_finish(
                first_value=result, second_value=None
            )
            check_duration = None
            if answer == True:
            ...
            answer = users_state[c.message.chat.id].search_request_data.append_date_or_period_to_finish(
                first_value=None, second_value=result
            )
            check_duration = None
            if answer == True:
    ...
    def first_period_calendar_handler(callback_query):
        ...
        bot.send_message(callback_query.message.chat.id, text=f"Выбери дату окончания периода.", reply_markup=calendar)
    ...
    def period_calendar_handler(callback_query):
        ...
        bot.send_message(callback_query.message.chat.id, text=f"Выбери дату или период вылета.", reply_markup=calendar)
    ...
    def circle_handler(callback_query):
        ...
        try:
            ...
            bot.reply_to(callback_query.message, "Выбери дату последнего возвратного вылета.", reply_markup=calendar)
    ...
    @bot.message_handler(
        func=lambda message: message.chat.id in users_state
        and users_state[message.chat.id].state == UserStates.WAIT_FOR_FINISH_AIRPORT
    )
...
def refresh_states(users_state):
    ...
    for user in Users.select():
        users_state[user.user_id] = UserState(user_id=user.user_id)
    for user_airport in UserAirports.select():
        user_id = user_airport.user_id
        users_state[user_id].search_request_data.append_airport(user_airport.airport)
    for string in UsersTranzit.select():
